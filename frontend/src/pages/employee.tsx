...
const Employee = () => {
  ...
  function GenderToSelectItems(Gender: { ID: number; Name: string }[]): ItemList[] {
  ...
  function PositionToSelectItems(Position: { ID: number; Name: string }[]): ItemList[] {
  ...
  async function onValid(formData: z.infer<typeof formEmployee>) {
    ...
    const res = await http.Post<Employees, Employees>("/employee", employeeData);
    if (res.ok) {
      toast({
        title: "You submitted the following values:",
        description: (
          <pre className="mt-2 w-[340px] rounded-md bg-slate-950 p-4">
            <code className="text-white">{JSON.stringify(employeeData, null, 2)}</code>
          </pre>
        ),
        duration: 1500,
      });
        ...
        fields={({ form }) => (
          ...
          <div className="flex flex-col ">
            ...
            <Label className="flex text-2xl mt-6 text-primary mx-64 ">
              ...
              <Form.Input className="w-3/4 h-14 px-4 ml-12 border rounded-md text-1xl focus:outline-none bg-white focus:border-black" useForm={form} name="LastName" type="text"></Form.Input>
            </Label > 
            ...
            <div className="flex">
              <Label className=" text-2xl text-primary mx-64 flex mt-6">
                ...
                <div className=" px-16 ">
                  <Form.DatePicker className="w-96 h-14 text-xl text-primary" useForm={form} name="DayOfBirth" />
                </div>
              </Label>
            </div>
            ...
            <Label className="flex text-primary text-2xl mx-64 mt-8">
              ...
              <Form.Input className="w-3/4 h-14 px-4 ml-12 border rounded-md text-1xl focus:outline-none bg-white focus:border-black" useForm={form} name="Phone" type="text"></Form.Input>
            </Label>
            <Label className="flex text-2xl mt-6 text-primary mx-64 ">
